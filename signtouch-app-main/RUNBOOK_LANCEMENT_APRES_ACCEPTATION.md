...
- [x] **Stripe** — ✅ **VÉRIFIÉ LE 05/08** sur le compte Plyz (`acct_1SzE31AVRRCVL5ZQ`).
  🔴 **Trou trouvé et bouché** : dans *Paramètres → Notifications d'équipe*, les deux catégories (« Demandes relatives à l'état du compte » et « Action requise concernant le virement ») étaient à **« — »**, donc **aucun destinataire**. Personne n'aurait été prévenu si Stripe exigeait une action sur le compte ou si les virements se bloquaient — les célébrités auraient cessé d'être payées sans signal. Corrigé : `contact@clickzou.fr` sur les deux. ⚠️ Ce champ ne propose que les **membres de l'équipe Stripe** : pour y mettre `jc@clickzou.fr`, il faudrait d'abord l'inviter comme membre (ce qui donnerait aussi un **second accès au compte Stripe**, aujourd'hui unique point de rupture sur l'argent — même sujet que le second admin App Store Connect).
  ✅ *Préférences en matière de communication → Transactions et Soldes* : déjà correctement réglé, notamment **« Litiges ou résolutions en matière de paiement »** (le plus critique : 7 jours pour répondre, sinon perte automatique), fraude Radar, solde négatif, virements initiés, remboursements inattendus.
  ⚪ « Paiements à risque élevé » **non cochable** (dépend vraisemblablement d'une option Radar payante) — sans conséquence, la détection Radar est active. « Paiements réussis » et « Nouveaux clients » laissés OFF volontairement : un e-mail par vente devient ingérable à l'échelle.
