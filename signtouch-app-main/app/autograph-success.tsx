...
export default function AutographSuccessScreen() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const { t } = useLanguage();
  const { autograph_id, session_id } = useLocalSearchParams<{ autograph_id: string; session_id: string }>();
      ...
      <View style={styles.center}>
        {verifying ? (
          ...
          <>
            ...
            <Text style={styles.title}>{t('autographSuccess') || 'Request sent!'}</Text>
            <Text style={styles.subtitle}>
              {t('autographSuccessHint') || 'The celebrity will fulfill your autograph request'}
            </Text>

            <View style={styles.infoCard}>
              <PenTool size={20} color="#f59e0b" />
              <Text style={styles.infoText}>
                {t('autographSuccessInfo') || 'The celebrity will create a personalized autograph for you. You will be notified when it is ready to view.'}
              </Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  infoCard: {
    ...
    borderWidth: 1, borderColor: 'rgba(245,158,11,0.15)', marginBottom: 32, width: '100%',
  },
  infoText: { color: '#d1d5db', fontSize: 14, flex: 1, lineHeight: 20 },
  goBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 8,
    backgroundColor: '#f59e0b', paddingVertical: 16, paddingHorizontal: 32,
