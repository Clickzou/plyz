...
export default function EventPublishScreen() {
  ...
  // prolongation. On garde donc la fin effective à part, renseignée par le
  // bandeau de prolongation (horloge et appel serveur y sont désormais logés).
  const [effectiveEndsAt, setEffectiveEndsAt] = useState<string | null>(null);
  const location = params.location as string;
  // ⚠️ NE PAS se fier au paramètre de navigation pour le prix. Les deux écrans qui
  // mènent ici le lisaient sur `event.price_cents` / `createdSession.price_cents` —
  // une colonne qui N'EXISTE PAS dans `event_sessions` (le prix n'est stocké que
  // dans event_payment_configs côté serveur, et en AsyncStorage côté appareil).
  // Ils envoyaient donc toujours 0, et la capture des paiements — conditionnée à
  // `priceCents > 0` — n'était JAMAIS déclenchée : le fan payait, la célébrité
  // publiait sa dédicace, et l'argent restait bloqué en pré-autorisation jusqu'à
  // expiration. Source de vérité = le serveur.
  const paramPriceCents = parseInt(params.priceCents as string || '0', 10);
  const [priceCents, setPriceCents] = useState(paramPriceCents);
  useEffect(() => {
    if (!sessionId || !STRIPE_SERVER_URL) return;
    (async () => {
      try {
        const r = await fetch(`${STRIPE_SERVER_URL}/api/get-event-payment-config?event_session_id=${sessionId}`);
        const cfg = await r.json();
        if (typeof cfg?.priceCents === 'number' && cfg.priceCents !== paramPriceCents) {
          console.log('[EventPublish] prix serveur:', cfg.priceCents, '(param:', paramPriceCents, ')');
          setPriceCents(cfg.priceCents);
        }
      } catch (e) {
        console.warn('[EventPublish] prix serveur indisponible, on garde le paramètre:', e);
      }
    })();
  }, [sessionId]);
  ...
  const handlePublish = async (type: 'photo' | 'photo_signed') => {
    ...
    try {
      ...
      if (type === 'photo_signed' && priceCents > 0 && STRIPE_SERVER_URL) {
        (async () => {
          try {
            const r = await authedFetch(`${STRIPE_SERVER_URL}/api/capture-event-payments`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ eventSessionId: sessionId }),
            });
            // `fetch` ne lève PAS d'exception sur un 403/500 : sans cette lecture
            // explicite, un refus du serveur passait totalement inaperçu. C'est
            // ainsi qu'une capture ratée est restée invisible — le fan gardait son
            // argent bloqué et la célébrité n'était jamais payée, sans un mot.
            const body = await r.json().catch(() => null);
            if (!r.ok) {
              console.error('[EventPublish] CAPTURE REFUSÉE par le serveur:', r.status, body);
            } else {
              console.log('[EventPublish] Capture:', body);
              if (body && typeof body.failedCount === 'number' && body.failedCount > 0) {
                console.error('[EventPublish] CAPTURE PARTIELLE —', body.failedCount, 'fan(s) non encaissé(s)');
              }
            }
          } catch (captureErr) {
            console.error('[EventPublish] Capture paiements échouée (réseau):', captureErr);
          }
        })();
      }
        ...
        <View style={styles.eventInfoCard}>
          ...
          {endsAt && (
            <View style={styles.eventInfoRow}>
              ...
              <Text style={styles.eventInfoText}>{t('endsAt') || 'Fin'}: {formatDateTime(effectiveEndsAt || endsAt)}</Text>
            </View>
          )}
        </View>
