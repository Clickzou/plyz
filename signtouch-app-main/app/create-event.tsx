...
import { getCurrentCoords, reverseGeocodeName } from '@/utils/geofence';
...
export default function CreateEventScreen() {
  ...
  const handleCreateEventAfterRestore = async (formData: any) => {
    ...
    // Dédicace = événement EN PERSONNE : position GPS du lieu TOUJOURS obligatoire.
    if (!formData.eventCoords) {
      showAlert(
        t('locationRequiredTitle' as any) || "Lieu de l'événement requis",
        t('locationRequiredMsg' as any) || "Cet événement a lieu en personne : définis le lieu avec « Partager ma position » sur place. Tes fans devront être présents pour recevoir leur dédicace."
      );
      return;
    }
  ...
  const handleCaptureLocation = async () => {
    ...
    try {
      ...
      if (coords) {
        ...
        // Remplit automatiquement le nom du lieu (optionnel, pour l'affichage aux
        // fans) — la célébrité n'a pas à le saisir. On respecte un nom déjà tapé.
        const placeName = await reverseGeocodeName(coords);
        if (placeName) setEventLocation((prev) => (prev && prev.trim() ? prev : placeName));
        if (Platform.OS !== 'web') {
          Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
        }
      } else {
  ...
  const performCreateEvent = async (connectedAccountId?: string) => {
    ...
    // Dédicace = événement EN PERSONNE : la position GPS du lieu est TOUJOURS
    // obligatoire (impose le géofence côté fan pour les événements payants ;
    // conformité Apple real-world / hors IAP).
    if (!eventCoords) {
      showAlert(
        t('locationRequiredTitle' as any) || "Lieu de l'événement requis",
            ...
            <>
              ...
              <View style={styles.ideasCard}>
                <View style={styles.ideasHeaderRow}>
                  <Star size={20} color="#f9a8d4" />
                  <Text style={styles.ideasTitle}>{t('dedicationIdeasTitle' as any) || 'Qui peut organiser une dédicace ?'}</Text>
                </View>
                <Text style={styles.ideasIntro}>{t('dedicationIdeasIntro' as any) || 'Une dédicace, c\'est une photo personnalisée et signée que tes fans reçoivent. Quelques idées pour t\'inspirer :'}</Text>

                <View style={styles.ideaRow}>
                  <Text style={styles.ideaEmoji}>🎤</Text>
                  <Text style={styles.ideaText}>{t('dedicationIdeaStar' as any) || 'Une star qui dédicace des photos signées à ses fans'}</Text>
                </View>
                <View style={styles.ideaRow}>
                  <Text style={styles.ideaEmoji}>⚽</Text>
                  <Text style={styles.ideaText}>{t('dedicationIdeaTeam' as any) || 'Un club de sport ou une équipe pour ses supporters'}</Text>
                </View>
                <View style={styles.ideaRow}>
                  <Text style={styles.ideaEmoji}>✍️</Text>
                  <Text style={styles.ideaText}>{t('dedicationIdeaArtist' as any) || 'Un artiste ou un auteur en séance de dédicace'}</Text>
                </View>
                <View style={styles.ideaRow}>
                  <Text style={styles.ideaEmoji}>💡</Text>
                  <Text style={styles.ideaText}>{t('dedicationIdeaExpert' as any) || 'Un expert ou un créateur qui remercie sa communauté'}</Text>
                </View>
              </View>
              ...
              <View style={styles.section}>
                <View style={styles.sectionHeaderRow}>
                  ...
                  <Text style={styles.sectionTitle}>{t('inPersonLocationTitle' as any) || 'Lieu de l\'événement (en personne)'}</Text>
                  <Text style={styles.requiredHint}>{t('requiredField') || '(requis)'}</Text>
                </View>
                <Text style={styles.locationGpsHint}>
                  {t('inPersonEventHint' as any) || 'Cet événement a lieu en personne. Partage ta position sur le lieu : tes fans devront être physiquement présents sur place pour recevoir leur dédicace.'}
                </Text>
                <TouchableOpacity
                ...
                >
                  ...
                  <Text style={[styles.locationGpsButtonText, eventCoords && styles.locationGpsButtonTextSet]}>
                    {eventCoords
                      ...
                      : (t('useMyLocation' as any) || 'Partager ma position sur le lieu')}
                  </Text>
                </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  signatureSvg: { flex: 1 },
  signaturePlaceholder: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 28, pointerEvents: 'none' },
  signaturePlaceholderText: { fontSize: 16, color: 'rgba(255,255,255,0.4)', textAlign: 'center', lineHeight: 24 },
